...
class MPCPlanner(BasePlanner):
    ...
    def __init__(self,  
        ...
        **kwargs
        ...
        # TODO: don't give evaluator to sub_planner?? make it optional
        self.sub_planner = hydra.utils.instantiate(
            ...
            log_filename=None,
        ) # Note: evaluator is shared for mpc and sub_planner
    ...
    def _apply_success_mask(self, actions):
        ...
        actions[mask] = 0
        masked_actions = rearrange(actions[mask], "... (f d) -> ... f d", f=self.evaluator.frameskip)
    ...
    def plan(self, obs_0, obs_g, actions=None):
        ...
        while not np.all(self.is_success) and self.iter < self.max_iter:
            ...
            logs, successes, e_obses, e_states = self.evaluator.eval_actions(action_so_far, self.action_len, filename=f'plan{self.iter}')
            new_successes = successes & ~self.is_success  # Identify new successes
            self.is_success = self.is_success | successes  # Update overall success status # TODO: is this needed???
            self.action_len[new_successes] = (self.iter + 1) * self.n_taken_actions # Update only for the newly successful trajectories
