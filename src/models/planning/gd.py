    ...
    def init_actions(self, obs_0, actions=None):
        ...
        if remaining_t > 0:
            if self.sample_type == "randn":
                ...
                new_actions = rearrange(new_actions, "... (f d) -> ... f d", f=self.evaluator.frameskip)
    ...
    def plan(self, obs_0, obs_g, actions=None):
        ...
        for i in range(self.opt_steps):
            ...
            with torch.no_grad():
                ...
                actions_new += torch.randn_like(actions_new) * self.action_noise # Add Gaussian noise
                actions.copy_(actions_new)

            self.wandb_run.log({f"{self.logging_prefix}/loss": total_loss.item(),
                                "step": i + 1})
            if self.evaluator is not None and i % self.eval_every == 0:
                logs, successes, _, _ = self.evaluator.eval_actions(actions.detach(), filename=f"{self.logging_prefix}_output_{i+1}")
