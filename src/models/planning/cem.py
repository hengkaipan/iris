...
class CEMPlanner(BasePlanner):
    ...
    def init_mu_sigma(self, obs_0, actions=None):
        ...
        '''
        # TODO: currently fixing sigma regardless of whether action is provided
    ...
    def plan(self, obs_0, obs_g, actions=None):
        ...
        for i in range(self.opt_steps):
            ...
            for traj in range(n_evals):
                cur_trans_obs_0 = {key: repeat(arr[traj].unsqueeze(0), '1 ... -> n ...', n=self.num_samples) for key, arr in trans_obs_0.items()}
                cur_z_obs_g = {key: repeat(arr[traj].unsqueeze(0), '1 ... -> n ...', n=self.num_samples) for key, arr in z_obs_g.items()}
                action = (torch.randn(self.num_samples, self.horizon, self.action_dim).to(self.device) * sigma[traj] + mu[traj])
            ...
            self.wandb_run.log({f"{self.logging_prefix}/loss": np.mean(losses),
                    "step": i + 1})
            if self.evaluator is not None and i % self.eval_every == 0:
                logs, successes, _, _ = self.evaluator.eval_actions(mu, filename=f"{self.logging_prefix}_output_{i+1}")
