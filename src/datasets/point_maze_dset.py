...
class PointMazeDataset(TrajDataset):
    def __init__(
        ...
        goals = torch.load(self.data_path / "goals.pth").float()
        if with_target:
            self.states = torch.cat([states, goals], dim=-1) # concatenate states and goals and dim = 6
        else:
            self.states = states
    ...
    def get_frames(self, idx, frames):
        obs_dir = self.data_path / "no_marker_obses" if not self.with_target else self.data_path / "obses"
