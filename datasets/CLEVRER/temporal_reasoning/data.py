...
from datasets.CLEVRER.temporal_reasoning.utils import prepare_relations, convert_mask_to_bbox, crop, encode_attr
from datasets.CLEVRER.temporal_reasoning.utils import normalize, check_attr, get_identifier, get_identifiers
from datasets.CLEVRER.temporal_reasoning.utils import check_same_identifier, check_same_identifiers, check_contain_id
from datasets.CLEVRER.temporal_reasoning.utils import get_masks, check_valid_masks, check_duplicate_identifier
from datasets.CLEVRER.temporal_reasoning.utils import rand_float, init_stat, combine_stat, load_data, store_data
from datasets.CLEVRER.temporal_reasoning.utils import decode, make_video
...
class PhysicsCLEVRDataset(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        for i in range(
            ...
            # import pdb; pdb.set_trace()
            img = cv2.resize(img, (self.W, self.H), interpolation=cv2.INTER_AREA).astype(float) / 255.
        ...
        # attr: torch tensor [3, 5, 24, 24] [n_objects, self.args.attr_dim, self.bbox_size, self.bbox_size]
        # x 
    ...
    def get(self, idx):
        ...
        for i in range(
            idx_frame - n_his * frame_offset,
            idx_frame + frame_offset + 1, frame_offset): # looping through multiple frames
            ...
            print("### i: ", i, "frame_filename: ", frame_filename)
            ...
            for j in range(n_objects):
                ...
                pos = normalize(pos, pos_mean, pos_std) # [2, 24, 24] TODO: ?
                # print(pos)
                mask_crop = normalize(crop(mask, bbox, self.H, self.W), 0.5, 1).unsqueeze(0) # [1, 24, 24]
                img_crop = normalize(crop(img, bbox, self.H, self.W), 0.5, 0.5).permute(2, 0, 1) # cropped img of obj with size bbox. [3, 24, 24]

                identifier = get_identifier(objects[j]) # e.g. ('yellow', 'rubber', 'cylinder')
                ...
                s = torch.cat([mask_crop, pos, img_crop], 0).unsqueeze(0), identifier # ( [1, 6, 24, 24], (some strings...) )
                object_inputs.append(s)
        ...
        try:
            feats = torch.cat(feats, 0) # [n_obj, 24, 24, 24]
        except:
            print(idx_video, idx_frame)
        # print("feats shape", feats.size())
        ...
        # x: [n_obj, 18, 24, 24]
        # label_obj: [6, 6, 24, 24]
        # label_rel: [n_obj * n_obj, 1]
        return attr, x, rel, label_obj, label_rel

# -----------------------------------

class VisualCLEVRDataset(Dataset):  
 
    def __init__(self, args, transforms, phase):
        self.args = args
        self.phase = phase
        self.loader = default_loader
        self.data_dir = args.data_dir
    ...
    def read_valid_idx(self):
        ...
        self.valid_idx = []
        self.metadata = []
        fin = open(os.path.join(os.path.dirname(__file__), self.valid_idx_lst), 'r').readlines()
    ...
    def gen_valid_idx(self):
        ...
        self.valid_idx = []
        self.metadata = []
        fout = open(os.path.join(os.path.dirname(__file__), self.valid_idx_lst), 'w')
    ...
    def __getitem__(self, idx):
        ...
        for i in range(
            ...
            img = self.loader(os.path.join(self.data_dir, frame_filename))
            img = np.array(img)[:, :, ::-1].copy()
            img = cv2.resize(img, (self.W, self.H), interpolation=cv2.INTER_AREA).astype(float) / 255. # original img shape: (320, 480, 3)
            transformed_img = self.transform_image(img)[:3].unsqueeze(0) # (1, 3, 224, 224)
            transformed_img = transformed_img.type(torch.FloatTensor)
            imgs.append(transformed_img)
        imgs = torch.stack(imgs)
        imgs = imgs.squeeze(1) # [num_frames, 1, 3, 224, 224] -> [num_frames, 3, 224, 224]
        return imgs
